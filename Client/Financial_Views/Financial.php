...
<?php if(!$payment_manager->hasStripeConnectAccount()): //If the user does not have a stripe account token in our database?>
    ...
    <?php else:?>
        ...
        <?php if($payment_manager->hasExternalBankAccountForStripeAccount()):?>
                ...
                <?php else:?>
                    <div>You do not have a refunds pending</div>
                <?php endif;?>
        <?php else:?>

            <div>You do not have a bank account attached to your financial account. To receive payouts or refunds, you must attach a bank account.</div>
